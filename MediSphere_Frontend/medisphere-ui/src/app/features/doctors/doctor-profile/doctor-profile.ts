...
export class DoctorProfileComponent implements OnChanges
{
  ...
  confirmDeleteAccount() {
    ...
    const reason = this.deleteReason.trim() || 'Doctor requested profile deactivation';
    this.authService.deleteAccount(reason).subscribe({
      next: (res) => {
        this.deleting.set(false);
        this.showDeleteModal.set(false);
        this.toast.success(res?.message || 'Doctor profile has been deactivated.');
        this.authService.logout();
      },
      error: (err) => {
        this.deleting.set(false);
        this.toast.error(err?.error?.message || 'Failed to deactivate profile. Please contact support.');
      }
    });
  }
}
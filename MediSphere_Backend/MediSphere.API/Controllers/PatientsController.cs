    ...
    [FromBody] FamilyMemberDto dto)
{
    ...
    [HttpDelete("account")]
    [Authorize(Roles = "Patient")]
    public async Task<ActionResult<ApiResponse<object>>> DeleteAccount([FromServices] IAccountDeletionService accountDeletionService, [FromQuery] string reason = "User requested self-deletion")
    {
        var refId = int.Parse(User.FindFirst("referenceId")?.Value ?? "0");
        if (refId <= 0) return Unauthorized(ApiResponse<object>.Fail("Invalid patient session."));

        await accountDeletionService.DeletePatientAccountAsync(refId, reason);
        return Ok(ApiResponse<object>.Ok(null!, "Account successfully deactivated and anonymized."));
    }
}

        ...
        int appointmentId)
{
    ...
    [HttpDelete("account")]
    [Authorize(Roles = "Doctor")]
    public async Task<ActionResult<ApiResponse<object>>> DeleteAccount([FromServices] IAccountDeletionService accountDeletionService, [FromQuery] string reason = "Doctor requested self-deletion")
    {
        var refId = int.Parse(User.FindFirst("referenceId")?.Value ?? "0");
        if (refId <= 0) return Unauthorized(ApiResponse<object>.Fail("Invalid doctor session."));

        await accountDeletionService.DeleteDoctorAccountAsync(refId, reason);
        return Ok(ApiResponse<object>.Ok(null!, "Doctor account successfully deactivated and soft-deleted."));
    }
}

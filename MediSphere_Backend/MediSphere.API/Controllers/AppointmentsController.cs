...
public class AppointmentsController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<PagedResult<AppointmentDto>>>> GetMyAppointments([FromQuery] int page = 1, [FromQuery] int pageSize = 10)
    {
        ...
        if (string.IsNullOrEmpty(refIdClaim) || !int.TryParse(refIdClaim, out var refId) || refId <= 0)
        {
            return Ok(ApiResponse<PagedResult<AppointmentDto>>.Ok(new PagedResult<AppointmentDto>
            {
                Items = Enumerable.Empty<AppointmentDto>(),
                Page = page,
                PageSize = pageSize,
                TotalCount = 0
            }));
    ...
    [HttpDelete("{id}")]
    public async Task<ActionResult<ApiResponse<object>>> CancelAppointment(int id, [FromBody] CancelAppointmentDto? dto = null)
    {
        var role = User.FindFirst(ClaimTypes.Role)?.Value ?? "";

        // Use the reference-ID resolution for Patients and Doctors
        var idClaim = (role == "Patient" || role == "Doctor")
            ? User.FindFirst("referenceId")?.Value ?? User.FindFirst(ClaimTypes.NameIdentifier)?.Value
            : User.FindFirst(ClaimTypes.NameIdentifier)?.Value;

        var requestingId = int.Parse(idClaim ?? "0");

        await _appointmentService.CancelAppointmentAsync(id, requestingId, role, dto);

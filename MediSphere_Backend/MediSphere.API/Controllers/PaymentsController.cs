...
public class PaymentsController : ControllerBase
{
    ...
    public async Task<IActionResult> Webhook()
    {
        ...
        try
        {
            ...
            {
                var refundObj = root.GetProperty("payload").GetProperty("refund").GetProperty("entity");
                string paymentId = refundObj.GetProperty("payment_id").GetString() ?? string.Empty;
                string refundId = refundObj.GetProperty("id").GetString() ?? string.Empty;
                decimal amountInPaise = refundObj.GetProperty("amount").GetDecimal();
                decimal amount = amountInPaise / 100.0m;

                await ProcessRefundProcessedAsync(paymentId, refundId, amount);
            }
            else if (eventType == "refund.failed")
            {
                ...
                await ProcessRefundFailedAsync(paymentId, refundId, "Provider reported refund failure via webhook.");
            }
    ...
    private async Task<bool> ProcessSuccessfulPaymentAsync(string orderId, string paymentId, decimal amount)
    {
        ...
        // Update Database States with Race-Condition Guard (Rule 8)
        ...
        {
            _logger.LogWarning("Payment arrived for ALREADY CANCELLED Appointment {Id}. Marking as Refund Processing...", appointment.Id);
        ...
        _logger.LogInformation("Payment successful recorded. Appointment ID: {AptId}, TransId: {TransId}", appointment.Id, transaction.Id);

        // If the appointment was already cancelled when payment arrived, auto-trigger refund to provider immediately
        if (isAlreadyCancelled)
        {
            try
            {
                var refundResult = await _paymentService.InitiateRefundAsync(
                    paymentId,
                    amount,
                    "Automated Refund: Payment captured after appointment cancellation");
                ...
                if (refundResult.Success)
                {
                    ...
                    bool isCompleted = refundResult.Message?.ToLower() == "processed"
                        || refundResult.Message?.ToLower() == "refunded"
                        || refundResult.RefundId?.StartsWith("sim_") == true;
                    if (isCompleted)
                    {
            ...
            {
                _logger.LogError(refundEx, "Failed to auto-refund late payment on cancelled appointment {Id}", appointment.Id);
            }
        }
        ...
        if (appointment.Patient != null)
        {
            ...
            await _backgroundTaskQueue.QueueBackgroundWorkItemAsync(async ct =>
            {
                ...
                if (patientId.HasValue)
                {
                    await signalR.NotifyQueuePositionChangedAsync(patientId.Value, queueToken, "Waiting");
                }
            });
        }
    ...
    private async Task ProcessRefundProcessedAsync(string paymentId, string refundId, decimal amount)
    {
        var tx = await _unitOfWork.Repository<PaymentTransaction>().Query()
            .Include(t => t.Appointment)
            .FirstOrDefaultAsync(t => t.RazorpayPaymentId == paymentId);

        if (tx == null)
        {
            _logger.LogWarning("refund.processed: No transaction found for PaymentId {PaymentId}", paymentId);
            return;
        }

        // Idempotency guard: skip if already marked Refunded
        if (tx.RefundStatus == RefundStatus.Refunded || tx.RefundStatus == RefundStatus.Simulated)
        {
            _logger.LogInformation("refund.processed: Transaction {TxId} already reconciled. Skipping duplicate webhook.", tx.Id);
            return;
        }

        tx.RefundStatus = RefundStatus.Refunded;
        tx.RazorpayRefundId = refundId;
        tx.RefundCompletedAt = DateTime.UtcNow;
        if (tx.RefundAmount == 0) tx.RefundAmount = amount;
        await _unitOfWork.Repository<PaymentTransaction>().UpdateAsync(tx);

        if (tx.Appointment != null)
        {
            tx.Appointment.PaymentStatus = "Refunded";
            await _unitOfWork.Repository<Appointment>().UpdateAsync(tx.Appointment);
        }
        await _unitOfWork.SaveChangesAsync();
        _logger.LogInformation("Reconciled refund.processed for PaymentId {PaymentId}, RefundId {RefundId}", paymentId, refundId);
    }

    private async Task ProcessRefundFailedAsync(string paymentId, string refundId, string reason)
    {
        var tx = await _unitOfWork.Repository<PaymentTransaction>().Query()
            .Include(t => t.Appointment)
            .FirstOrDefaultAsync(t => t.RazorpayPaymentId == paymentId);
        ...
        if (tx == null)
        {
            _logger.LogWarning("refund.failed: No transaction found for PaymentId {PaymentId}", paymentId);
            return;
        }

        // Idempotency guard
        if (tx.RefundStatus == RefundStatus.RefundFailed)
        {
            _logger.LogInformation("refund.failed: Transaction {TxId} already marked failed. Skipping duplicate webhook.", tx.Id);
            return;
        }

        tx.RefundStatus = RefundStatus.RefundFailed;
        tx.RefundFailureReason = reason;
        await _unitOfWork.Repository<PaymentTransaction>().UpdateAsync(tx);

        if (tx.Appointment != null)
        {
            tx.Appointment.PaymentStatus = "Refund Failed";
            await _unitOfWork.Repository<Appointment>().UpdateAsync(tx.Appointment);
        }
        await _unitOfWork.SaveChangesAsync();
        _logger.LogWarning("Reconciled refund.failed for PaymentId {PaymentId}, RefundId {RefundId}", paymentId, refundId);
    }
}

...
public static class EmailTemplates
{
    ...
    {
        return BuildAppointmentCancelledWithRefundEmail(patientName, doctorName, date, time, reason, null, null, null);
    }

    public static string BuildAppointmentCancelledWithRefundEmail(
        string patientName, string doctorName, string date, string time, string? reason,
        decimal? refundAmount, string? refundId, string? refundStatus)
    {
        var isRefundApplicable = refundAmount.HasValue && refundAmount.Value > 0;

        var refundSection = isRefundApplicable
            ? $@"<tr><td class='info-label'>Refund Amount</td><td class='info-colon'>:</td><td class='info-value'>&#8377;{refundAmount:N2}</td></tr>
                 <tr><td class='info-label'>Refund ID</td><td class='info-colon'>:</td><td class='info-value'>{refundId ?? "Processing"}</td></tr>
                 <tr><td class='info-label'>Refund Status</td><td class='info-colon'>:</td><td class='info-value'><span class='badge badge-success'>{refundStatus ?? "Initiated"}</span></td></tr>"
            : "<tr><td class='info-label'>Payment</td><td class='info-colon'>:</td><td class='info-value'>No payment captured (No refund due)</td></tr>";
        ...
        var content = $@"
            ...
            {(isRefundApplicable ? "<p>Your refund has been initiated and will reflect in your original payment method within 5-7 business days depending on your bank.</p>" : "")}
    ...
    public static string BuildDoctorAppointmentCancelledEmail(
        string doctorName, string patientName, string date, string time, string? reason = null, string? cancelledBy = null)
    {
        var content = $@"
        <p>Hello <strong>Dr. {doctorName}</strong>,</p>
        ...
        <div class='info-box'>
            <table class='info-table'>
                ...
                <tr><td class='info-label'>Date</td><td class='info-colon'>:</td><td class='info-value'>{date}</td></tr>
                <tr><td class='info-label'>Time</td><td class='info-colon'>:</td><td class='info-value'>{time}</td></tr>
                {(string.IsNullOrWhiteSpace(cancelledBy) ? "" : $"<tr><td class='info-label'>Cancelled By</td><td class='info-colon'>:</td><td class='info-value'>{cancelledBy}</td></tr>")}
                {(string.IsNullOrWhiteSpace(reason) ? "" : $"<tr><td class='info-label'>Reason</td><td class='info-colon'>:</td><td class='info-value'>{reason}</td></tr>")}
                <tr><td class='info-label'>Status</td><td class='info-colon'>:</td><td class='info-value'><span class='badge badge-danger'>Cancelled</span></td></tr>
            </table>
        </div>
        <p>This slot is now released in your clinical schedule. Your dashboard has been updated accordingly.</p>";
    ...
    public static string BuildRefundProcessedEmail(
        string patientName, string doctorName, string appointmentDate, decimal refundAmount,
        string refundId, string refundDestination, DateTime processedAt)
    {
        var content = $@"
        <p>Hello <strong>{patientName}</strong>,</p>
        <p>Your payment refund has been <strong>successfully processed</strong> with the banking provider.</p>
        <div class='info-box'>
            <table class='info-table'>
                <tr><td class='info-label'>Refund Amount</td><td class='info-colon'>:</td><td class='info-value'>&#8377;{refundAmount:N2}</td></tr>
                <tr><td class='info-label'>Refund Reference ID</td><td class='info-colon'>:</td><td class='info-value'>{refundId}</td></tr>
                <tr><td class='info-label'>Destination</td><td class='info-colon'>:</td><td class='info-value'>{refundDestination}</td></tr>
                <tr><td class='info-label'>Doctor</td><td class='info-colon'>:</td><td class='info-value'>Dr. {doctorName}</td></tr>
                <tr><td class='info-label'>Appointment Date</td><td class='info-colon'>:</td><td class='info-value'>{appointmentDate}</td></tr>
                <tr><td class='info-label'>Processed Date</td><td class='info-colon'>:</td><td class='info-value'>{processedAt:dd-MMM-yyyy HH:mm} UTC</td></tr>
                <tr><td class='info-label'>Status</td><td class='info-colon'>:</td><td class='info-value'><span class='badge badge-success'>Refunded</span></td></tr>
            </table>
        </div>
        <p>Depending on your financial institution or card issuer, the credited amount will appear in your statement within 1-3 business days.</p>
        <p>Thank you for your patience and for choosing MediSphere! &#128153;</p>";

        return BuildHtmlTemplate("Refund Processed", "Payment Refund Successfully Processed", content,
            "https://medisphere.app/appointments/history", "View Appointment History");
    }

    public static string BuildRefundFailedAdminAlertEmail(
        string adminName, int appointmentId, string patientName, decimal refundAmount, string failureReason)
    {
        var content = $@"
        <p>Hello <strong>{adminName}</strong>,</p>
        <p><strong style='color:#dc2626;'>Alert:</strong> An automated payment refund transaction has failed and requires admin intervention.</p>
        <div class='info-box'>
            <table class='info-table'>
                <tr><td class='info-label'>Appointment ID</td><td class='info-colon'>:</td><td class='info-value'>#{appointmentId}</td></tr>
                <tr><td class='info-label'>Patient</td><td class='info-colon'>:</td><td class='info-value'>{patientName}</td></tr>
                <tr><td class='info-label'>Refund Amount</td><td class='info-colon'>:</td><td class='info-value'>&#8377;{refundAmount:N2}</td></tr>
                <tr><td class='info-label'>Failure Diagnostics</td><td class='info-colon'>:</td><td class='info-value' style='color:#dc2626;'>{failureReason}</td></tr>
                <tr><td class='info-label'>Status</td><td class='info-colon'>:</td><td class='info-value'><span class='badge badge-danger'>Failed</span></td></tr>
            </table>
        </div>
        <p>Please log in to the MediSphere Refund Command Center to inspect gateway logs and trigger a manual retry.</p>";

        return BuildHtmlTemplate("Refund Action Required", "Payment Refund Failed Alert", content,
            "https://medisphere.app/admin/refunds", "Go to Refund Command Center");
    }

    public static string BuildAccountDeletedEmail(string userName, string userRole, DateTime deletedAt)
    {
        var content = $@"
        <p>Hello <strong>{userName}</strong>,</p>
        <p>This is a formal confirmation that your MediSphere <strong>{userRole}</strong> account has been permanently deactivated and your personally identifiable information has been removed/anonymized per healthcare data privacy guidelines.</p>
        <div class='info-box'>
            <table class='info-table'>
                <tr><td class='info-label'>Account Role</td><td class='info-colon'>:</td><td class='info-value'>{userRole}</td></tr>
                <tr><td class='info-label'>Deactivation Time</td><td class='info-colon'>:</td><td class='info-value'>{deletedAt:dd-MMM-yyyy HH:mm} UTC</td></tr>
                <tr><td class='info-label'>Data Status</td><td class='info-colon'>:</td><td class='info-value'><span class='badge badge-success'>PII Anonymized</span></td></tr>
                <tr><td class='info-label'>Financial/Audit Records</td><td class='info-colon'>:</td><td class='info-value'>Preserved for Statutory Compliance</td></tr>
            </table>
        </div>
        <p>All active login sessions and security tokens have been revoked. If you wish to use MediSphere again in the future, you are welcome to create a brand new account.</p>
        <p>We are grateful for your time with MediSphere and wish you the very best in your health journey.</p>";

        return BuildHtmlTemplate("Account Deactivated", "Account Deletion Confirmation", content);
    }

    public static string BuildPatientAccountDeletedAdminAlertEmail(
        string adminName, string patientName, string patientEmail, string reason)
    {
        var content = $@"
        <p>Hello <strong>{adminName}</strong>,</p>
        <p>A patient account has been deleted/anonymized on MediSphere.</p>
        <div class='info-box'>
            <table class='info-table'>
                <tr><td class='info-label'>Patient Name</td><td class='info-colon'>:</td><td class='info-value'>{patientName}</td></tr>
                <tr><td class='info-label'>Original Email</td><td class='info-colon'>:</td><td class='info-value'>{patientEmail}</td></tr>
                <tr><td class='info-label'>Reason</td><td class='info-colon'>:</td><td class='info-value'>{reason}</td></tr>
                <tr><td class='info-label'>Timestamp</td><td class='info-colon'>:</td><td class='info-value'>{DateTime.UtcNow:dd-MMM-yyyy HH:mm} UTC</td></tr>
            </table>
        </div>
        <p>Historical appointment records and financial ledger entries remain intact for reporting and audit integrity.</p>";

        return BuildHtmlTemplate("Patient Account Deleted", "Patient Deletion Alert", content,
            "https://medisphere.app/admin/patients", "View Patient Directory");
    }

    public static string BuildDoctorAccountDeletedAdminAlertEmail(
        string adminName, string doctorName, string doctorEmail, string specialty, string reason)
    {
        var content = $@"
        <p>Hello <strong>{adminName}</strong>,</p>
        <p>A doctor account has been deactivated/deleted on MediSphere.</p>
        <div class='info-box'>
            <table class='info-table'>
                <tr><td class='info-label'>Doctor</td><td class='info-colon'>:</td><td class='info-value'>Dr. {doctorName} ({specialty})</td></tr>
                <tr><td class='info-label'>Original Email</td><td class='info-colon'>:</td><td class='info-value'>{doctorEmail}</td></tr>
                <tr><td class='info-label'>Reason</td><td class='info-colon'>:</td><td class='info-value'>{reason}</td></tr>
                <tr><td class='info-label'>Timestamp</td><td class='info-colon'>:</td><td class='info-value'>{DateTime.UtcNow:dd-MMM-yyyy HH:mm} UTC</td></tr>
            </table>
        </div>
        <p>Future bookings have been safely resolved and past earnings/payout histories remain retained for compliance.</p>";

        return BuildHtmlTemplate("Doctor Account Deactivated", "Doctor Deletion Alert", content,
            "https://medisphere.app/admin/doctors", "View Doctor Directory");
    }
}